...
- **Per-profile content overrides**: edit any field (bullets, role, dates, skills…) and it is saved on the active profile only — other profiles keep the master wording. Overridden items are flagged and can be reverted individually (“Use master”) or all at once (“Clear customizations”). Editing a field back to the master value drops the override automatically.
...
- Because edits are copy-on-write, changing an item on a profile that overrides a field won't update the master. To change the shared wording, edit it on a profile without that override (or reset the override first).
